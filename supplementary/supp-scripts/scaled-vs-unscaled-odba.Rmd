...
*Outputs*: A figure for the manuscript is saved in "supplementary/supp-outputs/"
...
We were unable to apply a correction factor to data processed on board the device. To test whether tag accuracy influenced inter-individual differences in mean ODBA, we therefore used the raw accelerometry data to compare the corrected and uncorrected mean ODBA values across individuals.
...
In a separate script ('window-odba.R', available upon request), the scaling factors above were applied to each axis of the raw accelerometry data (i.e. raw_x * x_scaling_factor) before calculating mean ODBA at 10 minute intervals. We also repeated the same calculations but without applying the scaling factors. The resulting dataset is "activity_ODBA_10min-intervals-raw.csv".

*Note* The raw accelerometry data files are very large and only peripheral to this manuscript, so are not included with the data accompanying the manuscript. However, these data (stored in our database) and the script used to process these files ('window-odba.R') are available upon request.
...
odba <- read.csv(here("supplementary","supp-data","activity_ODBA_10min-intervals-raw.csv"))

odba <- odba %>%
  rename(mean_unscaled_ODBA=mean_ODBA)
```
