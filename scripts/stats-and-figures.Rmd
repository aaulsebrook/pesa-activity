...
daily_activity <- left_join(daily_activity, mean_fertile_females,
  ...
  mutate(ind_dev_ff=ind_mean_ff-n_FertileFemales)
```
...
* There is a fairly strong negative correlation between individual 'elevation' and 'slope'. So, males who were more active at their 'centred' number of fertile females aren't changing their activity as much as other males. Or, if we look at this the other way around, males with a lower centred activity level are increasing their activity more. This makes sense if the goal is to be more active than your rivals. But since variation in the slope is relatively small, I'm not sure how much to read into this. Also note that the nature of the relationship between intercept and slope (positive or negative) really depends on how the data are centred, suggesting that this relationship depends on where your reference point is.
