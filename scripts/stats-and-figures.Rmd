...
# Q: How does activity interact with breeding phase to predict success?

This doesn't really seem so interesting in the end.

## Model: Male activity, breeding phase and success

```{r}
# act_stage_m <- glmmTMB(success_num ~ breeding_stage*prop_activity +
#                          (breeding_stage|bird_ID),
#                    data=ind_activity,
#                    #data=daily_activity %>% filter(likely_undetected_success=="no"),
#                    family=poisson)
# summary(act_stage_m)
# confint(act_stage_m, level=0.95)
# 
# simulationOutput <- simulateResiduals(act_stage_m)
# testResiduals(simulationOutput)
```
...
nest_male_date_sub2 <- nest_male_date %>%
  filter(!nest %in% predated_nests$nest) %>%
  mutate(dist_m=as.numeric(dist_m),
         success=as.integer(success)) %>%
