...
activity_onboard <- read.csv(here("outputs", "processed-data",
                 glue("ODBA-and-activity_10min-intervals_onboard.csv")))
  # this dataset is used to help determine final data transmission

activity <- read.csv(here("outputs", "processed-data",
                 glue("ODBA-and-activity_{window_length}min-intervals_{data_type}.csv")))
...
## Calculate mean ODBA when active

To get a relative estimate of the "intensity" of activity, we can look at mean overall dynamic body acceleration during active periods.

```{r }
activity <- activity %>%
  mutate(mean_ODBA_active = case_when(active=="active" ~ mean_ODBA,
                                      TRUE ~ as.numeric(NA)))
```
...
Note that this next chunk of code can take a little while to run (determining centroid of points each date ~ 30 secs).
...
Note that this next chunk of code takes a while to run (getting distance from each male from each nest on each day; ~ 5 mins).
...
# However, mean ODBA and mean_ODBA_active should be calculated from activity
...
ind_odba <- activity %>%
  ...
  summarise(mean_ODBA=mean(mean_ODBA),
            mean_ODBA_active=mean(mean_ODBA_active, na.rm=T))
...
males_with_sufficient_data %>%
  ...
  select(colour_combo, bird_ID, n_days, n_samples, n_nests) %>%
  flextable()
```
...
stage_m <- glmmTMB(prop_activity ~ breeding_stage*success + (breeding_stage|bird_ID),
                   data=daily_activity,
...
When 'potentially' successful males are excluded the key results remain the same, although the interaction between breeding stage and success becomes statistically significant. There are no significant issues with model dispersion or influence of outliers.

```{r}
as.data.frame(emmeans(stage_m, ~ success | breeding_stage))
...
boxplot_active_date <- ggplot(daily_activity, aes(x=local_date,
  ...
  coord_flip(ylim=c(0,115),
             xlim = c(max(daily_activity$local_date)+24*60*30, # manually adjusted
...
# Q: TO RESOLVE: Within- vs. among-individual variation in activity

```{r}
# var_ff_m <- glmmTMB(prop_activity ~ 
#                   scale(n_FertileFemales) + 
#                   (scale(n_FertileFemales)|bird_ID),
#                   data = daily_activity,
#                   family = beta_family(link="logit"))
...
Daily male ODBA during activity ranged from less than 20% to over 99% (mean ± standard deviation: `r mean(daily_activity$perc_active)` +/- `r sd(daily_activity$perc_active)`.

```{r Difference in mean ODBA when active when min vs. max fertile females}
daily_activity %>%
  ungroup() %>%
  filter(n_FertileFemales==0 | n_FertileFemales==40) %>%
  select(colour_combo, mean_ODBA_active, n_FertileFemales) %>%
  group_by(colour_combo, n_FertileFemales) %>%
  summarise(mean_ODBA_active=mean(mean_ODBA_active)) %>%
  pivot_wider(values_from = 'mean_ODBA_active', names_from = 'n_FertileFemales', names_prefix="ff_") %>%
  filter(!is.na(ff_40) & !is.na(ff_0)) %>%
  mutate(diff=ff_40-ff_0) %>%
  flextable()
```

The table above shows the mean ODBA of males (each with a unique colour_combo) when they were active, when female fertility was at its peak (40 fertile females, 'ff_40') compared with when females were no longer fertile and were incubating (0 fertile females, 'ff_0'). All males showed more intense activity during the peak fertile period.
...
daily_activity %>%
  ungroup() %>%
  filter(n_FertileFemales==0) %>%
  summarise(sd_ODBA_active=sd(mean_ODBA_active),
...
*Prediction*: Males increase the intensity of their activity (mean ODBA when active) when there are fertile females at the site.

## Fig: Breeding stage and male active ODBA
...
odba_fert_boxplot <- ggplot() +
  ...
  scale_y_continuous(breaks=c(seq(0.2,0.6,by=0.1)),
                     expand=c(0,0))+
  scale_x_discrete(breaks = c("fertile box", "post-fertile box"),
                   labels = c("fertile","post-fertile"))+
  scale_colour_manual(values=c(fail_colour,success_colour), name="Sired offspring?")+
  ...
  scale_shape_manual(values=c(default_shape), guide="none") +
  xlab("\n breeding stage")+
  ylab("mean male ODBA when active\n")+
  theme(text=element_text(family=font.family),
        axis.title = element_text(size=axis.heading.size),
        axis.text = element_text(size=label.size),
        legend.title = element_text(size=legend.heading.size),
...
ind_activity %>%
  group_by(breeding_stage, success) %>%
  summarise(mean_mean_ODBA_active = mean(mean_ODBA_active),
            sd_ODBA_active = sd(mean_ODBA_active),
            n=n(),
...
Each datapoint represents the mean ODBA of an individual male when active.

```{r Check factors for model}
str(daily_activity$breeding_stage)
str(daily_activity$bird_ID)
```

```{r LME for breeding stage and percent male activity, width=10}
odba_stage_m <- glmmTMB(mean_ODBA_active ~ breeding_stage*success + (breeding_stage|bird_ID),
                   data=daily_activity,
                   #data=daily_activity %>% filter(likely_undetected_success=="no"),
                   family=beta_family(link="logit"))
...
When 'potentially' successful males are excluded the conclusions remain the same, although the interaction between breeding stage and success becomes statistically significant.

```{r}
as.data.frame(emmeans(odba_stage_m, ~ success | breeding_stage))
odba_stage_emm_flex <- as_flextable(as.data.frame(emmeans(odba_stage_m, ~ success | breeding_stage)))
...
Male tended to show more intense activity during the fertile stage, compared with the post-fertile stage. During the fertile period, successful males had more intense activity than unsuccessful males; during the post-fertile stage, there was no difference. The inclusion of males that were more likely to have sired additional offspring (predated before they could be sampled) had no effect on these results.

*Prediction 2* Intensity of male activity (mean ODBA when active) increases with the number of fertile females at the site.

We also examine this trend in more detail, by looking at activity and the number of fertile females each day across the season.

## Fig: Male active ODBA and number of fertile females

```{r LME for fertile females and male odba}
ff_m_odba <- glmmTMB(mean_ODBA_active ~ scale(n_FertileFemales) +
                     (scale(n_FertileFemales)|bird_ID),
                  data = daily_activity,
                  family = beta_family(link="logit"))
summary(ff_m_odba)
...
odba_ff_plot <- ggplot(daily_activity, aes(x=n_FertileFemales,
  ...
  scale_y_continuous(breaks=seq(0.2,0.8,by=0.1),
                     expand = c(0,0)) +
  xlab("\nnumber of fertile females\n")+
  ylab("mean daily ODBA when active\n")+
  theme(axis.ticks.length.y = unit(0.07, "cm"),
        text=element_text(family=font.family),
        axis.title = element_text(size=axis.heading.size),
        axis.text = element_text(size=label.size))
...
# TO FIX: Combined figure: Date and male active ODBA
...
# Combined figure: Fertile females and male active ODBA
...
Also note that mean ODBA during activity and proportion of time spent active are correlated.

```{r}
cor.test(ind_fertile_activity$mean_ODBA_active, ind_fertile_activity$prop_activity)
```
...
success_m2 <- glmmTMB(n_nests ~ scale(mean_ODBA_active) + scale(log(tenure_days)) + scale(arrival_day),
                   data=ind_fertile_activity,
                   # data=ind_fertile_activity %>% filter(likely_undetected_success=="no"),
                   family=poisson)
                   #family=gaussian)
...
m2_active_eff = m2_effects$`scale(mean_ODBA_active)` %>% data.frame
...
success_odba_plot <- ggplot(ind_fertile_activity)+
  geom_line(data = m2_active_eff,
            aes(y = fit, x = mean_ODBA_active),
            colour=col_scale[1]) +
  geom_ribbon(data = m2_active_eff,
              aes(y = fit, x = mean_ODBA_active, ymin = lower, ymax = upper),
              alpha = 0.2, fill=col_scale[1])+
  geom_jitter(aes(x=mean_ODBA_active, y=n_nests, shape=likely_extra_nests),
             fill=male_colour,
             colour=male_colour,
             size=point_size,
             alpha=point_alpha,
  ...
  theme_classic()+
  scale_shape_manual(values=c(default_shape, default_shape), guide="none")+
  scale_x_continuous(expand = c(0, 0), breaks=c(seq(0.2,0.6,by=0.1))) +
  scale_y_continuous(breaks=c(seq(0,2,by=1))) +
  coord_cartesian(xlim=c(0.23,0.63), ylim=c(0,2.1))+
  xlab("\n mean ODBA when active (g)") +
  ylab("mating success\n")+
  theme(legend.position="right",
        plot.margin=unit(c(0,0,0,0), 'lines'),
        text=element_text(family=font.family),
