...
*Outputs*: Tables and figures are saved in "outputs/figures-and-tables"

*Inputs*: Data inputs are found in "data" and "outputs/processed-data". Data processing steps are outlined below.
...
3. map-male-and-nest-locations.Rmd: Identifies when males took trips outside the study area

These processing scripts are found in the "scripts" folder. The key outputs are also stored in "outputs/processed-data". 
...
Most packages that are used in this Notebook can be installed using the install.packages function. One exception is "wadeR" which was developed by researchers at MPIO and needs to be installed from github.

```{r Install wadeR package}
# library(remotes)
...
library(egg) # useful for combining plots (ggarrange)
library(ggrepel) # for nicely spaced labels on plots
...
if(threshold_type=="global" & data_type!="onboard"){
  print("Warning: We recommend using the 'onboard' data when applying the global threshold. The global threshold was developed using onboard processed on board the device.")
}
...
activity_onboard <- read.csv(here("outputs", "processed-data",
                 glue("activity_odba_10min-intervals-onboard.csv")))
  # this dataset is used to help determine final data transmission

activity <- read.csv(here("outputs", "processed-data",
                 glue("activity_odba_{window_length}min-intervals-{data_type}.csv")))
...
We collected blood samples from all of the males we tagged (as well as some that we didn't) and from eggs/chicks on our study plot. DNA was extracted from these samples by collaborators at the University of Alaska. These DNA samples were shipped to Seewiesen. Genotyping was performed by Sylvia and parentage was analysed by Bart. 
...
GPS data need to be filtered down to data collected while the device was on the bird.

Occasionally, the devices recorded a latitude and longitude of 200. This is a default, error value that doesn't mean anything and should be excluded.
...
Some males left and then returned again. We will subtract the days that they were away from tenure.
...
There was also one male who travelled 300 km in one day (out over the ocean for 150km, then back; see map-male-and-nest-locations.Rmd). It doesn't make sense to include this data in our analysis. He was R,Y,DB and his travel date was 20 June.
...
daily_activity <- activity %>%
  ...
  summarise(perc_active=sum(active=="active")/n()*100,
            total_active=sum(active=="active"),
            mean_ODBA_active=case_when(perc_active==0 ~ NA,
                                       TRUE ~ mean(mean_ODBA[active=="active"])),
...
Note that this next chunk of code takes a while to run (getting distance from each male from each nest on each day).
...
In a separate script, I explored the three cases where males sired nests far from their territory. P1103 was sired by a male who had a territory further away, but did visit the nest site.
...
Males with a median daily distance <=200 metres from a nest site still seem to have a higher probability of siring the eggs. We can use this information to get a list of "likely" fathers of the predated nests.
...
males_with_sufficient_data = ind_activity %>%
  ...
  select(colour_combo, bird_ID, n_days, n_samples, success)

males_with_sufficient_data %>%
  arrange(n_days, n_samples, success) %>%
  flextable()
```
...
The percentage of time that males spent active in a day ranged from less than 20% to over 99% (mean ± standard deviation: `r mean(daily_activity$perc_active)` +/- `r sd(daily_activity$perc_active)`.
...
```{r Difference in activity when min vs. max fertile females 2}
daily_activity %>%
  ungroup() %>%
  filter(n_FertileFemales==0 | n_FertileFemales==40) %>%
  select(colour_combo, perc_active, n_FertileFemales) %>%
...
```{r Difference in activity when min vs. max fertile females 3}
daily_activity %>%
  ungroup() %>%
  filter(n_FertileFemales==0 | n_FertileFemales==40) %>%
  select(colour_combo, perc_active, n_FertileFemales) %>%
...
postfertile_males <- ind_activity %>%
  ...
  group_by(bird_ID) %>%
  summarise(mean(mean_perc_active))
...
daily_activity <- daily_activity %>%
  mutate(prop_activity=perc_active/100,
         bird_ID=as.factor(bird_ID),
         factor_date=as.factor(local_date),
...
stage_m <- glmmTMB(prop_activity ~ breeding_stage*success + (breeding_stage|bird_ID) + (1|factor_date),
                   data=daily_activity,
...
When 'potentially' successful males are excluded the key results remain the same. There are no significant issues with model dispersion or influence of outliers.

```{r}
stage_emm_flex <- as_flextable(as.data.frame(emmeans(stage_m, ~ success | breeding_stage)))

stage_emm_flex

if(save_tables==TRUE){
save_as_docx(stage_emm_flex, 
             pr_section = sect_properties,
             path = glue("{output_path}/breeding_stage_emm.docx"))
}
```

Male spent more time active during the fertile stage, compared with the post-fertile stage. During the fertile period, successful males were more active than unsuccessful males; during the post-fertile stage, there was no difference. The inclusion of males that were more likely to have sired additional offspring (predated before they could be sampled) had no effect on these results.

*Prediction 2* Male activity (% time active per day) increases with the number of fertile females at the site.

We can also examine this trend in more detail, by looking at activity and the number of fertile females each day across the season.
...
hor_barplot_ff <- barplot_ff +
  coord_flip(xlim = c(max(daily_activity$local_date)+24*60*30, #manually adjusted
                            min(daily_activity$local_date)-24*60*30),
             ylim=c(0,42)) +
    theme(panel.grid.minor.y = element_line(size=.1, color="grey"))+ 
    scale_x_continuous(breaks=c(seq(
                       min(daily_activity$local_date)-2*24*60*30,
                       max(daily_activity$local_date),
                       by="1 days")),
                       expand=c(0,0))+
...
We can separate within- and between- individual effects using within-subject centering (https://www.sciencedirect.com/science/article/pii/S0003347208005423?via%3Dihub).

```{r Determine mean activity for each individual}
mean_daily_activity <- daily_activity %>%
  mutate(hours_active=n_hours*prop_activity) %>%
  group_by(colour_combo, bird_ID) %>%
  summarise(ind_mean_prop_activity = mean(prop_activity),
            ind_mean_ODBA = mean(mean_ODBA),
            success = first(success),
            likely_undetected_success = first(likely_undetected_success),
            n_days = n(),
            n_hours = sum(n_hours),
            total_prop_activity = sum(hours_active)/sum(n_hours))

centred_daily_activity <- left_join(daily_activity, mean_daily_activity,
                            join_by('colour_combo','bird_ID',
                                    'success', 'likely_undetected_success'),
                            relationship='many-to-one') %>%
  mutate(perc_active = prop_activity-ind_mean_prop_activity)
```

```{r Model within and between individual effects}
ff_m2 <- glmmTMB(n_FertileFemales ~ scale(perc_active) + scale(ind_mean_prop_activity)  + (1|bird_ID),
                  data = centred_daily_activity)
summary(ff_m2)
confint(ff_m2, level=0.95)

simulationOutput <- simulateResiduals(ff_m2)
...
save_as_docx(ff2_flex, 
             pr_section = sect_properties,
             path = glue("{output_path}/fertile_females_model2.docx"))
}
ff2_flex
```

Relationship between male activity and number of fertile females attributed to within-individual changes.
...
odba_ff_rawbased_plot <- ggplot(daily_activity, aes(x=n_FertileFemales,
                                               y=mean_ODBA))+
  theme_classic()+
  #geom_boxplot(colour="gray30", outlier.shape = NA)+
  geom_point(size=small.point.size,
  ...
  scale_y_continuous(breaks=seq(0,1,by=0.2),
                     expand = c(0,0)) +
  xlab("\nnumber of fertile females\n")+
  ylab("mean daily ODBA\n")+
  theme(axis.ticks.length.y = unit(0.07, "cm"),
        text=element_text(family=font.family),
        axis.title = element_text(size=axis.heading.size),
        axis.text = element_text(size=label.size))

odba_ff_rawbased_plot
```
...
daily_activity %>%
  ...
  summarise(mean_perc_active = mean(perc_active),
            ...
            se_hours_active = 24/100*se_perc_active)
```
...
This can be analysed as a binary (yes/no) or numeric (number of females) outcome. I've included both, since I originally used a binary analysis and then changed it.

*Prediction 2* Increased time spent active (%) is associated with increased likelihood of mating success
*Prediction 3* Males who spend more days at the site during the fertile period have a higher probability of mating success
*Prediction 4* Males with higher mean ODBA have a higher probability of mating success.

In the plots and analyses below, I've only included data from the fertile stage, and only for males with at least `r min_acc_days` days of data during the fertile stage. 

However, I realised that it's a problem to exclude males with limited data for the tenure analysis, because then we're excluding all the males who stayed for less than `r min_acc_days`. So I am now also analysing the effects of tenure on mating success in a separate model, including the full dataset.

```{r Adapt data for analyses}
ind_activity <- ind_activity %>%
  mutate(prop_activity=total_perc_active/100,
         recording_hours=n_samples*window_length/60,
...
Tenure is highly correlated with number of hours of data recorded, which makes sense. So we should not include both these variables in the model. Tenure is the more interesting and biologically meaningful variable, so I'll focus on that.
...
write.csv(m1_effects,
          file=glue("{output_path}/activity-mating-effects_{window_length}min_{data_type}-{threshold_type}thresh.csv"))
...
```{r Export effects of tenure on mating success}
mt_effects = as.data.frame(confint(success_mt, level=0.95))

write.csv(mt_effects,
          file=glue("{output_path}/tenure-mating-effects_{window_length}min_{data_type}-{threshold_type}thresh.csv"))
...
Similar effects when males near predated nests are excluded.
...
write.csv(m3_effects,
          file=glue("{output_path}/activity-repr-effects_{window_length}min_{data_type}-{threshold_type}thresh.csv"))
...
```{r Export effects of tenure on reproductive success}
mt2_effects = as.data.frame(confint(success_mt2, level=0.95))

write.csv(mt2_effects,
          file=glue("{output_path}/tenure-reproduction-effects_{window_length}min_{data_type}-{threshold_type}thresh.csv"))
...
# Q: Does activity relative to competitors predict success at a given nest?

For this analysis, we don't want to limit the dataset just to males with at least 2 days of data.

But we do want to exclude nests that were predated before genotyping. 
...
```{r}
cor(nest_male_date_sub$z_prop_act, nest_male_date_sub$dist_m)
cor(nest_male_date_sub$prop_activity, nest_male_date_sub$dist_m)
cor(nest_male_date_sub$mean_ODBA, nest_male_date_sub$dist_m)
```
